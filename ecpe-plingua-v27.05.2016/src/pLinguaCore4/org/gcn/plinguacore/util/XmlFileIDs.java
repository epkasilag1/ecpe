...
public abstract class XmlFileIDs {
	...
	private String fileRoute = null;	//JM: fileRoute is null initally


	private static final String ROOT_DIRECTORY = "org/gcn/plinguacore/";
	private Map<String, Map<String, String>> ids;	//JM: this one maps

	protected XmlFileIDs(String fileRoute) {	//JM: the constructor
		if (fileRoute == null)
			throw new NullPointerException(
					"FileRoute argument shouldn't be null");
		this.fileRoute = ROOT_DIRECTORY+fileRoute;

		ids = new HashMap<String, Map<String, String>>();

		SAXBuilder builder = new SAXBuilder();	//JM: SaxBuilder is a jdom
		Document doc = null;
		 ...
		 */
		try {
			stream = this.getClass().getResourceAsStream("/" + this.fileRoute); //JM: what is "this"? getClass() and getResourceAsStream
	...
	private void addID(Element element, String expectedElementName,
		...
		if (!element.getName().equals(expectedElementName))	//JM: element.getName() is the name enclosed in tags
			throwFileException();
		ListIterator<?> iterator = element.getChildren().listIterator();	//JM: element within the element
		/*
	...
	protected void addID(Element element, String expectedElementName,
			String expectedElementChildName, Class<?> expectedParentClass) { //JM: this one hasa only 4 iterators, singleton is false by default
	...
	protected void checkElement(Element element, String expectedElementName,
			Class<?> expectedParentClass, boolean isSingleton) {	//JM: this one 
