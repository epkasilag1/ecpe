...
class ReadStandardRule extends XMLReadCellLikeRule {
	...
	private OuterRuleMembrane readOuterMembrane(Element om, String second)
		...
		int energy = 0;
		String attrEnergy = om.getAttributeValue("energy");
		energy = Integer.parseInt(attrEnergy);
		if(energy < 0){
			invalidValue("Energy", "less than 0");
		}
		...
		OuterRuleMembrane result = new OuterRuleMembrane(new Label(label), charge,energy,
				innerMultiSet, col);
	...
	private InnerRuleMembrane readSpecificInnerRuleMembrane(Element im)
		...
		String attrEnergy = im.getAttributeValue("energy");
		int energy = Integer.parseInt(attrEnergy);

		MultiSet<String> innerMultiSet = readSpecificMultiSet(im
				.getChild("multiset"));
		InnerRuleMembrane result = new InnerRuleMembrane(new Label(label), charge, energy,
				innerMultiSet);
