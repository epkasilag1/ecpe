...
public abstract class CellLikeSimulator extends AbstractSelectionExecutionSimulator {
	...
	public CellLikeSimulator(Psystem psystem) {
		...
		//System.out.println("CellLikeSimulator");
	

	}

	protected String getHeadAfter(ChangeableMembrane membrane) {
		...
		str += m.getId() + ", Label: " + m.getLabelObj() + ", Charge: "
				+ Membrane.getChargeSymbol(m.getCharge()) + ", Energy: " + Membrane.getEnergyString(m.getEnergy());

		//System.out.println("In the Simulator (CellLike), label = " + m.getLabel() + " energy = " + m.getEnergy());
		return str;
	}
	...
	protected String getHead(ChangeableMembrane membrane) {
		...
		if (m.isSkinMembrane())
			...
			str += "MEMBRANE ID: ";
		str += m.getId() + ", Label: " + m.getLabelObj() + ", Charge: "
				+ Membrane.getChargeSymbol(m.getCharge()) + ", Energy: " + Membrane.getEnergyString(m.getEnergyPrev());
	...
	@Override
	protected void printInfoMembrane(ChangeableMembrane membrane) {	//JM: I don't get it, why did it become a membrane
		//getInfoChannel().println("printInfoMembrane -- START");
		if (!(membrane instanceof CellLikeMembrane))
			throw new IllegalArgumentException("Illegal arguments");
		CellLikeMembrane m = (CellLikeMembrane)membrane;
		getInfoChannel().println("    " + getHeadAfter(m));	//JM: Compare charge, and energy
		getInfoChannel().println("    Multiset: " + m.getMultiSet());	//JM: Compare multisets ; in case of comparing prev to next configuration
		...
		//getInfoChannel().println("printInfoMembrane -- END");
		getInfoChannel().println();
	...
	@Override
	protected void printInfoMembraneShort(MembraneStructure membraneStructure) {	//JM: This one prints all the membranes??
		System.out.println("CellLikeSimulator :: printInfoMembraneShort membraneStructure");
		if (!(membraneStructure instanceof CellLikeSkinMembrane))
			throw new IllegalArgumentException("Illegal arguments");
		printInfoMembrane((CellLikeSkinMembrane)membraneStructure);	//JM: This one prints all the membranes??
	...
	protected void removeLeftHandRuleObjects(ChangeableMembrane membrane, IRule r,
		...
		//System.out.println("CellLikeSimulator: removeLeftHandRuleObjects");
		if (!(membrane instanceof CellLikeMembrane))
			throw new IllegalArgumentException("Illegal arguments");
		CellLikeMembrane m= (CellLikeMembrane)membrane;
